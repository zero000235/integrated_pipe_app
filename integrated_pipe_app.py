...
def solve_pipe_network(pipes_list, nodes_list, rho, mu, epsilon, q_sys_lmin, material_name):
    ...
    # 펌프 흡입관의 마찰손실을 실시간 판독하여, 공동현상이 원천 차단(NPSHa >= NPSHr + 0.8)될 때까지 직경을 자동 상향 튜닝
    pump_node_id = None
    for n in nodes_list:
        if n["type"] == "pump":
            pump_node_id = n["id"]
    ...
    if suction_pipe:
        ...
        # 마찰을 줄이기 위해 size_pool 내에서 순차 상향하며 Cavitation 모의 테스트
        best_d = curr_d
        best_npsha = -9999.0
        success = False
        ...
        for d_test in size_pool:
            if d_test < curr_d:
                continue
            v_test = calc_velocity(q_sys_m3s, d_test)
            ...
            if npsha_test >= npshr + 0.8:
                ...
                _, rec_spec = recommend_pipe_spec(q_sys_m3s, material_name)
...
def calc_friction_factor(Re: float, D: float, epsilon: float) -> tuple:
    ...
    if Re < 2300:
        ...
        # Haaland 약식 공식값을 Newton-Raphson의 초기 가정값(Initial guess)으로 스마트하게 활용!
        f_guess = 0.25 / (np.log10(rr / 3.7 + 5.74 / (Re**0.9)))**2
        
        # Newton-Raphson 수치해석으로 Colebrook-White 음함수 해의 1e-8 정밀 수렴 연산 개시
        # x = 1.0 / sqrt(f)에 대한 음함수 솔빙
        f = f_guess
        for _ in range(20):
            x = 1.0 / np.sqrt(f)
...
def main():
    ...
    with main_tabs[0]:
            ...
            try:
                ...
                if "nodes" in bridge_data and "pipes" in bridge_data:
                    ...
                    try:
                        ...
                        shared_json_input = tuning_package_js
        ...
        render_integrated_report(
            ...
            widget_key="canvas_json_bridge_t1", p_vapor=p_vapor, q_sys_lmin=q_sys_lmin, joint_method=joint_method
        )
